...
export const DashboardLayout = () => {
  ...
  const renderContent = () => {
    if (indicators.length === 0) {
      return (
        <div className="flex flex-col items-center justify-center h-64 text-center">
          ...
          <h2 className="text-xl font-medium mb-2">No indicators found</h2>
          <p className="text-slate-400">
            Try adjusting your search or filter criteria, or add a new indicator.
          </p>
        </div>
      );
    }
    ...
    switch (view) {
        ...
        return (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {indicators.map((indicator) => (
              ...
              >
                <h3 className="text-lg font-medium mb-1">{indicator.name}</h3>
                <p className="text-sm text-slate-400 mb-4">{indicator.description}</p>
                <div className="h-[250px]">
                  <IndicatorChart indicator={indicator} height={250} />
                </div>
              </div>
            ))}
          </div>
              ...
              <TableBody>
                {indicators.map((indicator) => (
                  <TableRow key={indicator.id}>
                    ...
                    <TableCell>{indicator.source.lastUpdated}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
      ...
      case "map":
        return (
          <div className="glass-morphism p-6 rounded-lg h-[calc(100vh-220px)]">
            <div className="flex items-center justify-center h-full">
              <div className="text-center">
                <h3 className="text-xl font-medium mb-3">Map View Coming Soon</h3>
                <p className="text-slate-400">
                  Geographic visualization of indicators is under development.
                </p>
              </div>
            </div>
          </div>
